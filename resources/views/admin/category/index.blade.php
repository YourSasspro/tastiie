...
@section('pageTitle', trans('gen.category'))

@section('content')
    <x-admin.container>
        <x-admin.container-header>
            <x-admin.module-title :title="__('gen.all', ['attribute' => trans('gen.category')])" />
            <x-admin.breadcrumb :currentPage="__('gen.category')" />
        </x-admin.container-header>
                ...
                <div class="row my-3">
                    <div class="col-md-6 mb-2">
                        <div class="input-search position-relative">
                            <x-buttons.add :route="route('admin.categories.create')" :title="__('gen.create', ['attribute' => __('gen.category')])" />
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        @foreach ($categories as $category)
                            <tr>
                                ...
                                <td class="align-middle text-start">
                                    <span
                                        class="badge {{ $category->status == 1 ? 'bg-success' : 'bg-danger' }} rounded-pill">
                                        {{ $category->status == 1 ? trans('gen.active') : trans('gen.inactive') }}
                                    </span>
                                </td>
                                <td class="align-middle text-start">
                                    <div>
                                        ...
                                        <div class="dropdown-menu p-2 ps-0" aria-genledby="dropdownMenuLink">
                                            ...
                                            <a class="dropdown-item" href="#">
                                                <x-buttons.del :route="route('admin.categories.destroy', $category->id)" />
                                            </a>
                                        </div>
                                    </div>
                                </td>
